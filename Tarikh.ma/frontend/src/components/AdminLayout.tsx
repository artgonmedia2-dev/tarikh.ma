        ...
        <div className="p-6 flex items-center gap-3">
          ...
          <div>
            <h1 className="text-sm font-bold leading-tight">
              {user.role === 'admin' ? 'Tarikh.ma Admin' : 'Tarikh.ma Contributeur'}
            </h1>
            <p className="text-[11px] text-slate-500 uppercase tracking-wider font-semibold">
              {user.role === 'admin' ? 'Portail Analytique' : 'Espace de Publication'}
            </p>
          </div>
        </div>
        <nav className="flex-1 px-4 space-y-2 mt-4">
          ...
          {user.role === 'admin' && (
            <>
              ...
              >
                ...
                <span className="text-sm font-medium">Héritage Section</span>
              </NavLink>
              <NavLink
                to="/admin/users"
                className={({ isActive }) =>
          ...
          <div className="pt-4 border-t border-slate-100 mt-4">
            ...
            {user.role === 'admin' ? (
              ...
              <div className="flex items-center gap-3 px-3 py-2 text-slate-400 cursor-not-allowed">
                <span className="material-symbols-outlined">help</span>
                <span className="text-sm font-medium">Aide & Guide</span>
              </div>
            )}
          </div>
        </nav>
